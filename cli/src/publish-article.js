...
/**
 * 发布图文文章。
 * 全程浏览器自动化：填标题、输入正文、上传封面、设置标签、发布/存草稿。
 */
export async function publishArticle(opts) {
  ...
  try {
    ...
    if (content) {
      const editorSelector = '[contenteditable="true"], .ProseMirror, .ql-editor, [class*="editor"] [contenteditable]';
    ...
    // ── 发布 / 草稿 ──
    if (opts.draft) {
      const draftBtn = await page.$('button:has-text("存草稿"), button:has-text("保存草稿"), [class*="draft"]');
      if (draftBtn) {

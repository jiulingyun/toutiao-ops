---
name: toutiao-ops
description: 今日头条创作者平台全流程运营自动化。登录管理、文章/视频/微头条发布、作品管理、评论管理、数据分析、创作灵感获取。使用浏览器自动化模拟真人操作发布内容，浏览器内 API 获取数据。当用户需要运营头条号、发布内容、查看数据或管理评论时使用此技能。
version: 1.0.0
icon: "\U0001F4F0"
metadata:
  clawdbot:
...
完整的头条号运营自动化工具，覆盖内容发布、数据分析、评论互动全流程。
...
所有命令支持 `--headless` 参数切换无头模式（默认有头模式）。
